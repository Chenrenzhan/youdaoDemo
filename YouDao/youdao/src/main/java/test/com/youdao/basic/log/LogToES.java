...
import test.com.youdao.basic.utils.DeviceUtils;
...
public class LogToES {
    ...
    /** In MB. */
    public static final int MAX_FILE_SIZE = 5;//修改日志最大文件为5M
    ...
    private static boolean containMinute(String fileName) {
        Matcher matcher = PATTERN_MINUTE.matcher(fileName);
        return matcher.find();
    }

    private static void writeLogToMinute(File dirFile, String path, String fileName, String msg,
                                         boolean immediateClose, long timeMillis) throws IOException {

        String lastFileName = CommonPref.instance().get(LAST_WRITE_LOG_FILE_TIME);
        int minute = getThreadCalendar().get(Calendar.MINUTE);
        String tempFileName;
        String tempFileTime_Hour;
        if(lastFileName == null) {
            tempFileName = fileName.substring(0, fileName.indexOf('.')) + "_" + minute + ".txt";
            CommonPref.instance().put(LAST_WRITE_LOG_FILE_TIME, tempFileName);
        } else {
            String lastFileTime_Hour = lastFileName.substring(0, lastFileName.lastIndexOf('_'));
            if(containMinute(fileName)) {
                tempFileTime_Hour = fileName.substring(0, fileName.lastIndexOf('_'));
            } else {
                tempFileTime_Hour = fileName.substring(0, fileName.indexOf('.'));
            }

            tempFileName = tempFileTime_Hour + "_" + minute + ".txt";
            File lastFile = new File(dirFile, lastFileName);
            if (lastFile.exists() && lastFileTime_Hour.equals(tempFileTime_Hour)) {
                long lastFileSize = (lastFile.length() >>> 20);
                if (lastFileSize >= MAX_FILE_SIZE && lastFileName.equals(tempFileName)) {
                    return;      //如果分钟内log日志文件内容超过20M，则不继续写log,递归结束
                } else if(lastFileSize < MAX_FILE_SIZE) {
                    tempFileName = lastFileName;   //如果分钟文件还可以写内容，则复用之前同一小时内的分钟文件，用来继续写log，减少创建log文件的数量
                } else {
                    CommonPref.instance().put(LAST_WRITE_LOG_FILE_TIME, tempFileName);
                }
            } else {
        ...
        writeLog(path, tempFileName, msg, immediateClose, timeMillis);
    }
    ...
    public static void writeLog(String path, String fileName, String msg,
        ...
        if (!logFile.exists()) {
            ...
            if (fileSize >= MAX_FILE_SIZE) {
                writeLogToMinute(dirFile, path, fileName, msg, immediateClose, timeMillis);
                return ;
            }
        }
    ...
    private static long getLogFileBackupTime(File file) {
        ...
        try {
            ...
            if (matcher.find()) {
                ...
                time = DateUtils.getSimpleDateFormat(BAK_DATE_FORMAT_STR).parse(dateStr).getTime();
